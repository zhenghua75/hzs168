...
function drupalgap_item_list_populate(list_css_selector, items) {
  try {
    ...
    setTimeout(function(){$(list_css_selector).listview('refresh').listview()},0);
...
function drupalgap_goto_generate_page_and_go(
  ...
  try {
    ...
    if(menu_link.page_tpl){
      if(drupalgap_function_exists(menu_link.page_tpl)){
        var fn = window[menu_link.page_tpl];
        page_template_path = fn();
      }
    }
...
function template_process_page(variables) {
  try {
    ...
    for (var index in drupalgap.theme.regions) {
        ...
        if(page_html.indexOf('{:' + region.name + ':}') <0) { continue; }
        var _region = {};
  ...
  try {
    ...
    if (drupalgap.settings.blocks[drupalgap.settings.theme][region.name]) {
      ...
      if (region.links && region.links.length > 0) {
        ...
        var left_data_role = 'controlgroup';
        if(region.attributes['left-data-role']){
          left_data_role = region.attributes['left-data-role'];
        }
        if (ui_btn_left_count > 1) {
          var attrs = {
            'data-type': 'horizontal',
            'data-role': left_data_role,
            'class': 'ui-btn-left'
          };
 ...
 * @param {Object} entity
 */
function drupalgap_entity_render_content(entity_type, entity,displayname) {
  try {
    entity.content = '';
    ...
    for (var field_name in field_info) {
        ...
        if (displayname && field.display[displayname]) {
            display = field.display[displayname];
            if (
              typeof display.module === 'undefined' &&
              typeof field.display['default'].module !== 'undefined'
            ) { display.module = field.display['default'].module; }
...
function entity_primary_key_title(entity_type) {
  try {
    ...
    switch (entity_type) {
      ...
      case 'commerce_store': key= 'title'; break;
      default:
        console.log(
...
function field_permissions(entity,name) {
  try {
    if (
      (
        entity.uid == Drupal.user.uid &&
        user_access('edit own ' + name)
      ) ||
      user_access('edit any ' + name)
    ) { return true; }
    else { return false; }
  }
  catch (error) { console.log('field_permissions - ' + error); }
}
        ...
        if (field_info) {
          ...
          if (entity && entity[name] && entity[name].length != 0 && entity[name][language]) {
            for (var delta = 0; delta < cardinality; delta++) {
              ...
              // value property.
              var fn = field_info.module + '_value_callback';
              if (drupalgap_function_exists(fn)) {
                default_value = window[fn](field_info,entity[name][language],delta);
              }
              else if (
          ...
          if(field_info['field_permissions'] && field_info['field_permissions']['type']){
            //0公有1私有  2自定义权限
            switch(field_info['field_permissions']['type']){
              case '0':
              case '1':
                break;
              case '2':
                form.elements[name]['access'] = field_permissions(entity,name);
                break;
              default:
                break;
            }
      ...
      case 'taxonomy_term_reference':
          ...
          switch(instance.widget.type){
            ...
            case 'options_buttons':
              ...
              items[delta].children.push({
                  markup: '<div id="'+items[delta].id + '-div"></div>'
              });
    ...
    switch (delta) {
      ...
      case 'powered_by':
        return '<p style="text-align: center;">' + t('Powered by') + ': ' +
          l('核作社', 'http://123.57.234.155', {InAppBrowser: true}) +
        '</p>';
        break;
      case 'help':
...
function taxonomy_assemble_form_state_into_field(entity_type, bundle,
  ...
  try {
    ...
    switch (instance.widget.type) {
      ...
      case 'options_buttons':
        field_key.use_wrapper = false;
        var values = form_state_value.split(',');
        var value =[];
...
function _select_selected(default_value,value){
  try{
    if($.isArray(default_value)){
      for (var i = 0; i < default_value.length; i++) {
        if(default_value[i]==value){
    ...
    console.log('_select_selected - ' + error);
  }
}
function _checkbox_checked(default_value,value){
  try{
    for (var i = 0; i < default_value.length; i++) {
      if(default_value[i]==value){
        return true;
    ...
    console.log('_checkbox_checked - ' + error);
  }
}
/**
          ...
          switch(options.type){
            case 'select':
              ...
              for (var index in terms) {
                  if (!terms.hasOwnProperty(index)) { continue; }
                  var term = terms[index];
                  var option = '<option value="' + term.tid + '" '+_select_selected(options.default_value,term.tid)+'>' +
                    term.name +
                  '</option>';
                  $(widget).append(option);
              ...
              break;
            case 'checkboxes':
              var widget = $('#' + options.widget_id+'-div');
              ...
              for (var index = terms.length - 1; index >= 0; index--) {
                  ...
                  var checkbox = {
                    value: term.tid,
                    attributes: {
                      id:options.widget_id + '-' + term.tid ,
                      name: options.widget_id + '-' + term.tid ,
                      'value':term.tid,
                      onchange: "_theme_taxonomy_term_reference_checkbox_onchange(this, '" + options.widget_id + "');"
                    }
                  };
                  var label = {
                    element:{
                  ...
                  html += theme('form_element_label',label);
                  if(_checkbox_checked(options.default_value,term.tid)){
                    checkbox['checked']=true;
                  }
                  html += theme('checkbox', checkbox);
...
function _theme_taxonomy_term_reference_checkbox_onchange(input, id) {
  try {
    //7,8,9,10
    var value = $('#' + id).val();
